...
def test_custom_list_generator():
    custom_list = CustomList()
    generator = Generator()
    hamster = generator.one_thousand_generator()
    for hamster in hamster:
        custom_list.append(hamster)
    assert len(custom_list) == 1000
    assert custom_list[0].name == hamster[0].name
    custom_list[0] = hamster[1]
    assert custom_list[0].name == hamster[1].name
    hamster_10000 = generator.ten_thousand_generator()
    custom_list_10000 = CustomList()
    for hamster in hamster_10000:
        custom_list_10000.append(hamster)
    for hamster in hamster_10000[::-1]:
        custom_list_10000.insert(0, hamster)
    for hamster in hamster_10000:
        custom_list_10000.append(hamster)
    center_hamster = custom_list_10000[len(custom_list_10000) // 2]
    for i, _ in enumerate(custom_list_10000):
        custom_list_10000[i] = center_hamster
